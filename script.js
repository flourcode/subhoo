...
let currentDisplayMode = 'subawards'; // Default or set based on initial load
let rawSubData = []; // Keep original name for subaward data compatibility
let rawPrimeData = []; // For temporarily holding raw prime data before adaptation
 ...
 * @param {Array<object>} rawPrimeDataSource - Array of objects parsed from uscc_primes.csv
 * @returns {Array<object>} - Array of objects with headers mapped to subaward format.
 */
function adaptPrimeDataToSubDataStructure(rawPrimeDataSource) {
    if (!rawPrimeDataSource || !Array.isArray(rawPrimeDataSource)) {
        return [];
    }
    console.log(`Adapting ${rawPrimeDataSource.length} prime award records...`);
