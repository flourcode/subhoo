    ...
    if (shareButton) {
        shareButton.addEventListener('click', async () => {
            const shareData = {
                ...
                text: 'Check out Subhoo - Federal Subcontractor Intelligence!', // Customize this text
            ...
            if (navigator.share) {
                try {
                    ...
                    // Avoid alerting for cancellation errors ('AbortError')
                ...
                // Optional: Alert user or try to copy link (more complex)
                 showNotification('Sharing not supported in this browser. You can copy the URL manually.', 'info'); 
                 // Example copy fallback (requires user interaction often)
                 /* try {
                     await navigator.clipboard.writeText(window.location.href);
                     showNotification('URL copied to clipboard!', 'info');
                 } catch (clipErr) {
                     console.error('Failed to copy URL:', clipErr);
                     showNotification('Sharing not supported and failed to copy URL.', 'warning');
                 } 
                 */
            }
        });
...
//     console.log("Updating data table view...");
    ...
    tableHeader.innerHTML = `
        ...
        <div class="data-table-controls">
            ...
            <button id="export-csv" class="btn btn-text">Export Full CSV</button>
        </div>
    `;
...
function updateActiveFiltersDisplay() {
    const filterContainer = document.getElementById('active-filters'); if (!filterContainer) { console.warn("Active filter display container (#active-filters) not found."); return; }
    const agencyLabel = getAgencyDisplayName();
    const displayValueText = formatCurrencyWithSuffix(minContractValue);
    const agencyTitleHTML = `<h2 class="agency-filter-title">${agencyLabel} FY25</h2>`;
	let otherFiltersHTML = ''; let hasOtherFilters = false;

    // Add Min Value filter
    otherFiltersHTML += `<span class="filter-item"><span class="filter-label">Showing&nbsp;Subcontracts&nbsp;>&nbsp;${displayValueText}</span>`; hasOtherFilters = true;

    // Add SubAgency filter if active (Added back)
    if (currentSubAgencyFilter) { otherFiltersHTML += `<span class="filter-separator">|</span> <span class="filter-item"><span class="filter-label">SubAgency:</span> <strong>${truncateText(currentSubAgencyFilter, 30)}</strong></span>`; hasOtherFilters = true; }

    // Add Office Filter
    if (currentOfficeFilter) { otherFiltersHTML += `<span class="filter-separator">|</span> <span class="filter-item"><span class="filter-label">Office:</span> <strong>${truncateText(currentOfficeFilter, 30)}</strong></span>`; hasOtherFilters = true; }

    // Add NAICS, Prime, Sub, Search filters
    if (currentNaicsFilter) { const el=document.getElementById('naics-filter');let nT=currentNaicsFilter,nFT=currentNaicsFilter;if(el){const opt=Array.from(el.options).find(o=>o.value===currentNaicsFilter);if(opt){nFT=opt.textContent;nT=opt.value;}}otherFiltersHTML+=`<span class="filter-separator">|</span> <span class="filter-item" title="<span class="math-inline">\{nFT\}"\><span class\="filter\-label"\>NAICS\:</span\> <strong\></span>{nT}</strong></span>`; hasOtherFilters = true; }
    if (currentPrimeFilter) { otherFiltersHTML += `<span class="filter-separator">|</span> <span class="filter-item"><span class="filter-label">Prime:</span> <strong>${truncateText(currentPrimeFilter, 30)}</strong></span>`; hasOtherFilters = true; }
    if (currentSubFilter) { otherFiltersHTML += `<span class="filter-separator">|</span> <span class="filter-item"><span class="filter-label">Sub:</span> <strong>${truncateText(currentSubFilter, 30)}</strong></span>`; hasOtherFilters = true; }
    if (currentSearchTerm) { otherFiltersHTML += `<span class="filter-separator">|</span> <span class="filter-item"><span class="filter-label">Search:</span> <strong>"${truncateText(currentSearchTerm, 20)}"</strong></span>`; hasOtherFilters = true; }

    // Update the container
    let finalHTML = agencyTitleHTML; if (hasOtherFilters) { finalHTML += `<div class="other-filters">${otherFiltersHTML}</div>`; }
    filterContainer.innerHTML = finalHTML;

    // Update console log (Include both SubAgency and Office)
//     console.log(`Updated Active Filters Display. Agency=<span class="math-inline">\{agencyLabel\}, View\=</span>{currentVizType}, Filters: SubAgency=<span class="math-inline">\{currentSubAgencyFilter \|\| 'N/A'\}, Office\=</span>{currentOfficeFilter || 'N/A'}, NAICS=<span class="math-inline">\{currentNaicsFilter \|\| 'N/A'\}, Prime\=</span>{currentPrimeFilter || 'N/A'}, Sub=<span class="math-inline">\{currentSubFilter \|\| 'N/A'\}, Search\=</span>{currentSearchTerm || 'N/A'}`);
}
...
//     console.log('Showing info panel for:', node?.name);
    ...
    titleElement.textContent = node?.name || 'Details Unavailable'; 
    contentElement.innerHTML = '';
    addInfoItem(contentElement, 'Type', capitalizeFirstLetter(node?.type || 'Unknown'));
    addInfoItem(contentElement, 'Total Value', formatCurrency(node?.value || 0));

    // Find more detailed info from processedData if available
    let permalink = '#';
    if (!processedData || !processedData.links || !processedData.nodes) {
        console.warn("Processed data not available for detailed info panel.");
        addInfoItem(contentElement, 'Details', 'Data incomplete.');
    } else {
        try {
            const allLinks = Object.values(processedData.links).flat();
            const allNodes = Object.values(processedData.nodes).flat();

            let relevantLinks = allLinks.filter(link => link?.source === node.id || link?.target === node.id);

            const linkWithPermalink = relevantLinks.find(link => link?.details?.permalink && link.details.permalink !== '#' && link.details.permalink.startsWith('http'));
            if (linkWithPermalink) permalink = linkWithPermalink.details.permalink;
            else console.log("No valid permalink found in connected links details.");
            ...
            // Display connections based on node type
            if (node.type === 'prime') {
                if (outgoingConnections.length > 0) {
                    addInfoSection(contentElement, 'Top Subcontractors');
                    outgoingConnections.slice(0, 5).forEach(conn => addInfoItem(contentElement, conn.name, formatCurrency(conn.value)));
                }
                else { addInfoItem(contentElement, 'Subcontractors', 'None found in current view.'); }
            } else if (node.type === 'sub') {
                 if (incomingConnections.length > 0) {
                     addInfoSection(contentElement, 'Prime Contractors');
                     incomingConnections.slice(0, 5).forEach(conn => addInfoItem(contentElement, conn.name, formatCurrency(conn.value)));
                 }
                 else { addInfoItem(contentElement, 'Prime Contractors', 'None found in current view.'); }
            } else { // Agency, Subagency, Office
                 if (incomingConnections.length > 0) {
                     const type = incomingConnections[0].type;
                     addInfoSection(contentElement, `Top Incoming (${capitalizeFirstLetter(type||'')})`);
                     incomingConnections.slice(0, 5).forEach(conn => addInfoItem(contentElement, conn.name, formatCurrency(conn.value)));
                 }
                 if (outgoingConnections.length > 0) {
                     const type = outgoingConnections[0].type;
                     addInfoSection(contentElement, `Top Outgoing (${capitalizeFirstLetter(type||'')})`);
                     outgoingConnections.slice(0, 5).forEach(conn => addInfoItem(contentElement, conn.name, formatCurrency(conn.value)));
                 }
            ...
            console.error("Error processing node details:", error); 
            addInfoItem(contentElement, 'Error', 'Could not load details.'); 
        }
    }

    linkElement.href = permalink;
    const isValidPermalink = permalink && permalink !== '#' && permalink.startsWith('http');
    linkElement.style.display = isValidPermalink ? 'inline-flex' : 'none';
...
// Load agency data from CSV
function loadAgencyData(agency) {
//     console.log(`Loading SUBCONTRACT data for agency: ${agency}`);
    showLoading();
        ...
        Papa.parse(dataUrl, {
            ...
            complete: function(results) {
                ...
                if (results.errors?.length > 0) {
                    ...
                    // Decide if you still want to proceed if there are errors
                }
                if (!results.data || results.data.length === 0) {
                    console.error(`No data loaded from ${dataUrl}`);
                    rawData = []; // Ensure rawData is empty
                    processedData = null; // Reset processed data
                    processData(); // Call processData to handle the empty state UI
                    hideLoading();
                ...
                // console.log("Sample Row:", rawData[0] || "No data"); // Keep commented unless debugging

               // Populate filter dropdowns based on raw data
                populateFilterDropdowns();

                // Now process the data for charts/tables etc.
                // (Call processData directly, NO setTimeout)
                processData();

                // Hide loading AFTER processing seems complete
                hideLoading();
            },
            error: function(error, file) {
                console.error(`Error loading CSV: ${file}`, error);
                rawData = []; // Ensure rawData is empty on error
                processedData = null; // Reset processed data
                processData(); // Call processData to handle the empty state UI
                hideLoading();
...
// Process data for visualizations - Re-integrated SubAgency correctly
...
//     console.log("Starting processData...");
    ...
    if (!rawData || rawData.length === 0) {
        ...
        displayTopNTable([], 'top-subs-container', 'Top Subcontractors', 10);
        displayNaicsTable([], 'naics-container', 'Top NAICS Codes', 5);
        if (currentVizType) updateVisualization(currentVizType);
        return;
    }

    // Calculate Unfiltered Aggregates (no change needed)
    const unfilteredPrimeMap = new Map(); const unfilteredSubMap = new Map(); const unfilteredNaicsValueMap = new Map();
    rawData.forEach((row) => { /* ... existing aggregation logic ... */
        const primeName = row['prime_awardee_name']?.trim() || 'Unknown Prime Contractor'; const subName = row['subawardee_name']?.trim() || 'Unknown Subcontractor'; const subValue = typeof row['subaward_amount'] === 'number' ? row['subaward_amount'] : parseFloat(String(row['subaward_amount'] || '0').replace(/[^0-9.-]+/g, '')) || 0; const naicsCode = row['prime_award_naics_code']?.toString().trim() || 'Unknown'; const naicsDesc = row['prime_award_naics_description']?.trim() || 'Unknown'; if (subValue <= 0) return; if (primeName !== 'Unknown Prime Contractor') { if (!unfilteredPrimeMap.has(primeName)) { unfilteredPrimeMap.set(primeName, { id: `prime-${primeName}`, name: primeName, type: 'prime', value: 0 }); } unfilteredPrimeMap.get(primeName).value += subValue; } if (subName !== 'Unknown Subcontractor') { if (!unfilteredSubMap.has(subName)) { unfilteredSubMap.set(subName, { id: `sub-${subName}`, name: subName, type: 'sub', value: 0 }); } unfilteredSubMap.get(subName).value += subValue; } if (naicsCode && naicsCode !== 'Unknown' && naicsCode !== 'N/A' && naicsCode !== '') { const key = `${naicsCode} - ${naicsDesc||'No Description'}`; unfilteredNaicsValueMap.set(key, (unfilteredNaicsValueMap.get(key)||0)+subValue); }
    });
    const unfilteredPrimeNodes = Array.from(unfilteredPrimeMap.values()).sort((a, b) => b.value - a.value); const unfilteredSubNodes = Array.from(unfilteredSubMap.values()).sort((a, b) => b.value - a.value); const unfilteredNaicsBreakdown = Array.from(unfilteredNaicsValueMap, ([k, v]) => { const p=k.split(' - '); return { code:p[0], desc:p.slice(1).join(' - ').trim()||'No Description', value:v }; }).sort((a,b)=>b.value-a.value);
    ...
    const agencyMap = new Map(), subAgencyMap = new Map(), officeMap = new Map(), primeMap = new Map(), subMap = new Map(); // Added subAgencyMap back
    const agencyToSubMap = new Map(), subToOfficeMap = new Map(), officeToContractorMap = new Map(), contractorToSubcontractorMap = new Map(); // Restored links

    let totalContractValue = 0; let totalPrimeContractsFiltered = new Set(); let totalSubContractsFiltered = new Set();
    const uniquePrimeNamesFiltered = new Set(); const uniqueSubNamesFiltered = new Set();
    const filteredNaicsValueMap = new Map();

    dataToProcess.forEach((row) => {
        const rawAgencyName = row['prime_award_awarding_agency_name']?.trim();
        const rawSubAgencyName = row['prime_award_awarding_sub_agency_name']?.trim(); // NEEDED
        const rawOfficeName = row['prime_award_awarding_office_name']?.trim();
        const rawPrimeName = row['prime_awardee_name']?.trim();
        const rawSubName = row['subawardee_name']?.trim();

        const agencyName = rawAgencyName || 'Unknown Agency';
        const subAgencyName = rawSubAgencyName || 'Unknown Sub-Agency'; // NEEDED
        const officeName = rawOfficeName || 'Unknown Office';
        const primeName = rawPrimeName || 'Unknown Prime Contractor';
        const subName = rawSubName || 'Unknown Subcontractor';

        const subValue = typeof row['subaward_amount'] === 'number' ? row['subaward_amount'] : parseFloat(String(row['subaward_amount'] || '0').replace(/[^0-9.-]+/g, '')) || 0;
        const primeValue = typeof row['prime_award_amount'] === 'number' ? row['prime_award_amount'] : parseFloat(String(row['prime_award_amount'] || '0').replace(/[^0-9.-]+/g, '')) || 0;
        const permalink = row['usaspending_permalink'] || '#';
        const naicsCode = row['prime_award_naics_code']?.toString().trim() || 'Unknown';
        const naicsDesc = row['prime_award_naics_description']?.trim() || 'Unknown Description';
        const popState = row['place_of_performance_state'] || row['pop_state'] || row['prime_award_pop_state'] || 'Unknown';

        // Skip row if essential hierarchy is missing OR value is zero
        if (subValue <= 0 || agencyName === 'Unknown Agency' || subAgencyName === 'Unknown Sub-Agency' || officeName === 'Unknown Office' || primeName === 'Unknown Prime Contractor' || subName === 'Unknown Subcontractor') return;

        // Stats Calculation
        totalContractValue += subValue; totalPrimeContractsFiltered.add(`${primeName}|${officeName}`); totalSubContractsFiltered.add(`${subName}|${primeName}`);
        if (primeName !== 'Unknown Prime Contractor') uniquePrimeNamesFiltered.add(primeName); if (subName !== 'Unknown Subcontractor') uniqueSubNamesFiltered.add(subName);

        // Node Creation (Restore SubAgency level & correct parent links)
        if (!agencyMap.has(agencyName)) agencyMap.set(agencyName, { id: `agency-${agencyName}`, name: agencyName, type: 'agency', value: 0, acronym: extractAcronym(agencyName) });
        agencyMap.get(agencyName).value += subValue;
        const canonicalAgencyName = agencyMap.get(agencyName).name;

        if (subAgencyName !== 'Unknown Sub-Agency') { // Add SubAgency back
            if (!subAgencyMap.has(subAgencyName)) subAgencyMap.set(subAgencyName, { id: `subagency-${subAgencyName}`, name: subAgencyName, type: 'subagency', value: 0, parent: canonicalAgencyName, acronym: extractAcronym(subAgencyName) });
            subAgencyMap.get(subAgencyName).value += subValue;
        }
        const canonicalSubAgencyName = subAgencyMap.has(subAgencyName) ? subAgencyMap.get(subAgencyName).name : 'Unknown Sub-Agency';

        if (officeName !== 'Unknown Office' && canonicalSubAgencyName !== 'Unknown Sub-Agency') { // Office parent is SubAgency
            if (!officeMap.has(officeName)) officeMap.set(officeName, { id: `office-${officeName}`, name: officeName, type: 'office', value: 0, parent: canonicalSubAgencyName });
             officeMap.get(officeName).value += subValue;
        }
        const canonicalOfficeName = officeMap.has(officeName) ? officeMap.get(officeName).name : 'Unknown Office';

        if (primeName !== 'Unknown Prime Contractor' && canonicalOfficeName !== 'Unknown Office') { // Prime parent is Office
            if (!primeMap.has(primeName)) primeMap.set(primeName, { id: `prime-${primeName}`, name: primeName, type: 'prime', value: 0, parent: canonicalOfficeName, popState: popState });
             primeMap.get(primeName).value += subValue;
        }
        const canonicalPrimeName = primeMap.has(primeName) ? primeMap.get(primeName).name : 'Unknown Prime Contractor';

        if (subName !== 'Unknown Subcontractor' && canonicalPrimeName !== 'Unknown Prime Contractor') { // Sub parent is Prime
             if (!subMap.has(subName)) subMap.set(subName, { id: `sub-${subName}`, name: subName, type: 'sub', value: 0, parent: canonicalPrimeName, popState: popState });
             subMap.get(subName).value += subValue;
        }

        // Link Creation (Restore original hierarchy)
        const agencyId=`agency-${canonicalAgencyName}`; const subAgencyId=subAgencyName!=='Unknown Sub-Agency'?`subagency-${canonicalSubAgencyName}`:null; const officeId=officeName!=='Unknown Office'?`office-${canonicalOfficeName}`:null; const primeId=primeName!=='Unknown Prime Contractor'?`prime-${canonicalPrimeName}`:null; const subId=subName!=='Unknown Subcontractor'?`sub-${subName}`:null;
        if(agencyId&&subAgencyId){const k=`${agencyId}|${subAgencyId}`;if(!agencyToSubMap.has(k))agencyToSubMap.set(k,{source:agencyId,target:subAgencyId,value:0,details:{sourceType:'agency',targetType:'subagency',permalink}});agencyToSubMap.get(k).value+=subValue;}
        if(subAgencyId&&officeId){const k=`${subAgencyId}|${officeId}`;if(!subToOfficeMap.has(k))subToOfficeMap.set(k,{source:subAgencyId,target:officeId,value:0,details:{sourceType:'subagency',targetType:'office',permalink}});subToOfficeMap.get(k).value+=subValue;}
        if(officeId&&primeId){const k=`${officeId}|${primeId}`;if(!officeToContractorMap.has(k))officeToContractorMap.set(k,{source:officeId,target:primeId,value:0,details:{sourceType:'office',targetType:'prime',naicsCode,naicsDesc,permalink,popState}});officeToContractorMap.get(k).value+=subValue;}
        if(primeId&&subId){const k=`${primeId}|${subId}`;const pct=(primeValue>0)?((subValue/primeValue)*100).toFixed(2)+'%':'N/A';if(!contractorToSubcontractorMap.has(k))contractorToSubcontractorMap.set(k,{source:primeId,target:subId,value:0,details:{sourceType:'prime',targetType:'sub',naicsCode,naicsDesc,subValue,primeValue,percentage:pct,permalink,popState}});contractorToSubcontractorMap.get(k).value+=subValue;}

        // Aggregate Filtered NAICS Data
        if (naicsCode && naicsCode !== 'Unknown' && naicsCode !== 'N/A' && naicsCode !== '') { const key = `${naicsCode} - ${naicsDesc}`; filteredNaicsValueMap.set(key, (filteredNaicsValueMap.get(key) || 0) + subValue); }
    }); // End dataToProcess.forEach

    // Finalize Filtered NAICS Breakdown
    const filteredNaicsBreakdown = Array.from(filteredNaicsValueMap, ([k, v]) => { const p=k.split(' - '); return { code:p[0], desc:p.slice(1).join(' - ').trim()||'No Description', value:v }; }).sort((a,b)=>b.value-a.value);
    let primaryNaics = { code:'N/A', desc:'', value:0 }; if(filteredNaicsBreakdown.length>0) primaryNaics=filteredNaicsBreakdown[0];

    // Store processed data (Added subagency back, restored original links)
    processedData = {
        nodes: { agency: Array.from(agencyMap.values()), subagency: Array.from(subAgencyMap.values()).sort((a,b)=>b.value-a.value), office: Array.from(officeMap.values()).sort((a,b)=>b.value-a.value), prime: Array.from(primeMap.values()).sort((a,b)=>b.value-a.value), sub: Array.from(subMap.values()).sort((a,b)=>b.value-a.value) },
        links: { agencyToSub: Array.from(agencyToSubMap.values()), subToOffice: Array.from(subToOfficeMap.values()), officeToContractor: Array.from(officeToContractorMap.values()), contractorToSubcontractor: Array.from(contractorToSubcontractorMap.values()) },
        stats: { totalContractValue, totalPrimeContracts: totalPrimeContractsFiltered.size, totalSubContracts: totalSubContractsFiltered.size, primaryNaics, filteredNaicsBreakdown, uniquePrimeContractorCount: uniquePrimeNamesFiltered.size, uniqueSubcontractorCount: uniqueSubNamesFiltered.size },
        unfilteredNodes: { prime: unfilteredPrimeNodes, sub: unfilteredSubNodes }, unfilteredNaics: unfilteredNaicsBreakdown
    };

    // --- Update UI ---
//     console.log("processData finished. Updating UI elements...");
    updateStatsDisplay(); updateDataTable();
    ...
    displayNaicsTable(processedData.unfilteredNaics, 'naics-container', 'Top NAICS Codes', 7); // Match donut N
    ...
    if (currentVizType) { updateVisualization(currentVizType); } else { applyFiltersAndDraw(); }
}
...
// Simplified version WITHOUT file existence checks
async function discoverAgencies() {
//     console.log("Discovering agencies (assuming all files exist)...");
    ...
    // --- Removed fileExists check ---
    const availableAgencies = [...agencies]; // Assume all are available

    if (availableAgencies.length > 0) {
        availableAgencies.sort((a, b) => a.label.localeCompare(b.label));
        ...
        const initialAgency = availableAgencies.find(a => a.value === currentAgency) ? currentAgency : availableAgencies[0].value;

        currentAgency = initialAgency; // Update global state if needed (though should match)
        agencySelector.value = initialAgency; // Set dropdown value
//         console.log(`Agency discovery complete (simplified). Initial agency: ${initialAgency}`);

        // *** The single initial load call ***
        loadAgencyData(initialAgency);

    } else {
        // This part is less likely to be reached now, but kept for structure
        console.error("No agencies defined in the code.");
        // ... (Error handling for empty agencies array) ...
    }
}
...
function updateExpiringContracts(data) {
    ...
    if (expiringTitleElement) {
        const agencyDisplayName = getAgencyDisplayName(); 
        expiringTitleElement.textContent = `Expiring ${agencyDisplayName} Contracts (> $200K ~6 Mo)`;
    } else {
